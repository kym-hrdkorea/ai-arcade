...
3. game registry의 `guide.slides`에 첫 화면 카드에서 열릴 규칙/사용설명 작성
4. 게임 카드와 `?` 사용설명이 허브에 표시되도록 연결
5. 라우트와 기본 로비 화면 생성
6. 필요한 실시간 이벤트 초안 작성
7. 최소 테스트 또는 테스트 계획 작성
8. README 또는 docs 업데이트

새 게임 설계 체크리스트:
- `guide.slides`는 실제 플레이 전 알아야 할 흐름과 핵심 규칙 중심으로 3~6개 작성할 것
- 각 사용설명 슬라이드는 짧은 title, 한 문단 body, items 2~4개를 포함할 것
- 상세 운영 정책, 서버 이벤트, 내부 구현 설명은 카드 설명이 아니라 GAME_SPEC.md에 둘 것
