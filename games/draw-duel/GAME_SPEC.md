...
```txt
인간 정답자: 정답 시 100점 × (1 + 0.5 × 남은시간비율) = 100~150점 (빠를수록 높음, 서버 시각 기준)
AI: 정답 시 +100점 (속도 보너스 없음)
출제자 +50점 규칙: 미구현 (별도 결정 전까지 보류)
```

개인 점수는 최종 "정답 랭킹"에 합산 점수로 표시하고, 팀 점수(HUMAN vs AI 승패)는
과반 정답 +100 규칙을 그대로 유지한다. 속도 보너스 상한을 1.5배로 낮게 잡은 이유는
행사장 네트워크 지연 편차가 순위를 좌우하지 않게 하기 위해서다 (2026-08-26 개정,
Real or AI와 동일한 공식).
